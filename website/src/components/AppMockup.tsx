...
function CountdownRing({ pct, color }: { pct: number; color: string }) {
  ...
  return (
    <svg width="36" height="36" className="-rotate-90">
      <circle cx={18} cy={18} r={r} fill="none" stroke="#E5E5E5" strokeWidth={2.5} />
      <circle
...
function FileCard({ file, hovered }: { file: typeof FILES[0]; hovered?: boolean }) {
  ...
  return (
    <div
      ...
      style={hovered ? { boxShadow: '0 8px 32px rgba(108,92,231,0.12), 0 2px 8px rgba(0,0,0,0.06)', transform: 'translateY(-2px)' } : { boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}
    >
      {file.pinned && (
      ...
      <div className="flex items-start gap-2.5 relative">
        {/* File icon */}
        <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: file.bgColor }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M3 2h7l3 3v9H3V2z" fill={file.color} fillOpacity="0.25" stroke={file.color} strokeWidth="0.8" />
            <path d="M10 2v3h3" fill="none" stroke={file.color} strokeWidth="0.8" />
          </svg>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-limbo-text mt-0.5 tabular-nums">{file.size}</p>
        </div>
      ...
      {hovered && (
        <div className="flex items-center gap-0.5 mt-2">
          {[Pin, Copy, Save, FolderOpen, Trash2].map((Icon, i) => (
            <div key={i} className="w-6 h-6 rounded-md flex items-center justify-center text-limbo-text/70 bg-limbo-muted">
              <Icon size={11} />
            </div>
          ))}
        </div>
...
export function AppMockup() {
  return (
    <div
      ...
      style={{
        background: '#F5F3EF',
        boxShadow: '0 32px 80px rgba(108,92,231,0.2), 0 8px 24px rgba(0,0,0,0.12), inset 0 0 0 1px rgba(255,255,255,0.6)',
      }}
    >
      {/* Title bar */}
      <div className="flex items-center justify-between px-4 h-11 border-b border-limbo-border/40" style={{ background: '#F5F3EF', cursor: 'default' }}>
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-lg gradient-btn flex items-center justify-center shadow-primary shrink-0">
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-limbo-border/30">
        {['Limbo', 'Settings'].map((tab, i) => (
          <div
            ...
            style={i === 0 ? { background: 'linear-gradient(135deg, #7B6CF6, #6C5CE7)', boxShadow: '0 2px 8px rgba(108,92,231,0.3)' } : {}}
          >
            {tab}
            {i === 0 && (
              <span className="bg-white/25 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">4</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-10 pointer-events-none" style={{ background: 'linear-gradient(to top, #F5F3EF, transparent)' }} />
    </div>
  )
}
